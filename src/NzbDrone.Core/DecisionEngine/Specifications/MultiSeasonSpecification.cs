...
namespace NzbDrone.Core.DecisionEngine.Specifications
{
    public class MultiSeasonSpecification : IDownloadDecisionEngineSpecification
    {
        ...
        {
            if (subject.ParsedEpisodeInfo.IsMultiSeason || subject.ParsedEpisodeInfo.IsCompleteSeries)
            {
                if (_configService.AllowMultiSeasonPacks)
                {
                    ...
                    _logger.Debug("Multi-season or complete-series release {0} allowed by config", subject.Release.Title);
                    return DownloadSpecDecision.Accept();
                }

                _logger.Debug("Multi-season or complete-series release {0} rejected. Not supported", subject.Release.Title);
